...
$\bar{{\cal C}}$, we evaluate the following semi-supervised cut-off strategies:
\begin{itemize}
...
\item \textit{Global cut:} the threshold is chosen globally over all blocks,
    as the one maximizing some score $f(\bar{{\cal C}}, \widehat{\bar{{\cal C}}}_t)$.
\item \textit{Block cut:} the threshold is chosen locally at each block $b$,
    as the one maximizing some score $f(\bar{{\cal C}}^b, \widehat{\bar{{\cal C}}^b_t})$.
    In case $\bar{{\cal C}}^b$ is empty, then all signatures from $b$ are clustered together.
\end{itemize}
Where $\widehat{\bar{{\cal C}}}_t$ denotes the predicted clusters, restricted to the
signatures in $\bar{{\cal C}}$, when cutting the hierarchy at threshold $t$, and
where $\bar{{\cal C}^b}$ is limited to signatures of $\bar{{\cal C}}$ in block $b$.

\glnote{Notations should be improved and be made more explicit}

...
clusters $c_a$, each including all signatures $s$ for which $(s, a) \in {\cal C}$.
...
${\cal C}$, we evaluate three cut-off strategies:
\begin{itemize}
\item No cut: all signatures from the same block are assumed to be from the same author;
\item Global cut: the threshold is chosen globally over all blocks, as the one maximizing some score $f({\cal C}, \hat{{\cal C}}_t)$;
\item Block cut: the threshold is chosen locally at each block $b$, as the one maximizing some score $f({\cal C}^b, \hat{{\cal C}}^b_t)$;
\end{itemize}
where $\hat{{\cal C}}_t$ denotes the predicted clusters,
restricted to the signatures in ${\cal C}$, when cutting the hierarchy at threshold $t$, and
where ${\cal C}^b$ is the subset of ${\cal C}$ including only those signatures from block $b$.

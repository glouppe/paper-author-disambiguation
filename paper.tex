...
{\cal S}$. Following previous works, pairs of signatures $(s_1, s_2)$ are
transformed to vectors $v \in \mathbb{R}^p$ by building so-called similarity
profiles \citep{treeratpituk2009disambiguating} on which supervised learning is
then carried on. In this work, we design and evaluate 15 standard input features based
on the comparison of signature fields, as reported in Table~\ref{table:features}.
As an illustrative example, the \textit{Full name} feature corresponds to ...
...
\begin{tabular}{|l|l|}
  \hline
  \textbf{Feature} & \textbf{Comparator}\\
  \hline
  \hline

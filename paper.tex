...
signatures $(s_1, s_2)$ are therefore first transformed to vectors $v \in
\mathbb{R}^p$ by building so-called similarity profiles
...
author name fields of the two signatures, as measured using the cosine
similarity between their respective TF-IDF vector representations as
comparator. Similarly, the \textit{Year difference} feature measures the
...
built a logistic regression classifier for mapping the TF-IDF representation of

...
this work, the setting extends with the partial knowledge of pairs ${\cal C} = \{ (a, s) \in
{\cal A} \times {\cal S} \}$, each indicating that signature $s$ is known with high confidence
to belong to individual $a$. Equivalently, ${\cal C}$ can be seen as a set of partial
clusters $c_a$, each including all signatures $s$ for which $(s, a) \in {\cal C}$. \glnote{Change notations for ${\cal C}$!!}
...
distinct authors.  Exploiting knowledge from the partial clusters known in
${\cal C}$, we evaluate the following semi-supervised cut-off strategies:
\begin{itemize}
\item \textit{No cut:} all signatures from the same block are assumed to be from the same author.
\item \textit{Global cut:} the threshold is chosen globally over all blocks, as the one maximizing some score $f({\cal C}, \hat{{\cal C}}_t)$.
\item \textit{Block cut:} the threshold is chosen locally at each block $b$, as the one maximizing some score $f({\cal C}^b, \hat{{\cal C}}^b_t)$. In case ${\cal C}^b$ is empty, then
                 all signatures from $b$ are clustered together.
\end{itemize}
Where $\hat{{\cal C}}_t$ denotes the predicted clusters, restricted to the
signatures in ${\cal C}$, when cutting the hierarchy at threshold $t$, and
where ${\cal C}^b$ is the subset of ${\cal C}$ including only those signatures
from block $b$.

...
Gilles Louppe\\
       ...
       \email{g.louppe@cern.ch}
\alignauthor
Hussein T. Al-Natsheh\\
       \affaddr{ISH-LYON (USR 3385), CNRS, France}\\
...
Eamonn Maguire\\
       \affaddr{CERN, Switzerland}\\
       \email{eamonn.james.maguire@cern.ch}
}
...
\cite{newman2001structure}. Thus, even small improvements in the field significantly improve the
+usability of the digital libraries to some users.
...
\underline{Name normalization} & 0.9901 & 0.9760 & \textbf{0.9830} & 0.9948 & 0.9738 & \textbf{0.9842}  \\
\hline
\hline
...
\hline
\hline
Combined best settings & 0.9897 & 0.9827 & \textbf{0.9862}  & 0.9954 & 0.9805 & \textbf{0.9879} \\
  \hline
\end{tabular}
\end{table*}

Our approach could be considered as an experimental design solution. Our goal is to find the best combination of design parameters. Such a solution could be reached by performing many experiments where in each experiment we fix all design parameters except one. Of course, what we found as a best choice in each experiment will be used for the proceeding one and so on. To initialize the process, we will use what have been reported by some related works as a best method.

\textit{state-of-the-art.} Unfortunately, most methods proposed in related works have released neither their software, nor their data, making in practice a fair comparison very difficult. However, we believe state-of-the-art algorithms proposed in the literature could still be shown by categorizing the design parameters into the three categories; blocking strategy, linkage function, and clustering and compare the choices per each category. This categorization represents the three main phases in the pipeline. We believe that results for different settings somehow reflects some of these state-of-the-art choices as we will show in this section. Results presented in Table ~\ref{table:results} are discussed with
respect to a state-of-the-art using the following parameter design setting per category. In Table ~\ref{table:results}, what we considered as state-of-the-art choice is underlined which are as follows:

\begin{itemize}
	\item Blocking: same surname and the same first given name initial strategy (SFI) and with name normalization;
...
\textit{Baseline.} Using machine learning techniques should perform better than dealing with the problem with other simpler method. Accordingly, we have set our experiment baseline having neither linkage function nor clustering. Actually, This is the case of many digital library where they use a simple and very fast execution solution with acceptable performance. Accordingly, our baseline algorithm is simply to group the set of publications from author names that have common surname and first initial as denoted in section \ref{methods:blocking} by \textit{SFI} and label them with a distinct author. However, this baseline also considers \textit{Name Normalization} for the 5 cases that is also previously discussed in section \ref{methods:blocking}. The results of the baseline is reported in the top of  Table ~\ref{table:results}.

In sections \ref{choices:blocking}, \ref{choices:linkage}, and \ref{choices:clustering}, we will discuss each group of design parameters in respect to the state-of-the-art choice of that group. Keep in mind that the results of each group uses the best of the other design parameters.
...
\subsubsection{Combined best settings} When all best settings are combined (\ie, Blocking = NYSIIS, Name normalization, Classifier = Random Forests, Training

...
\begin{table*}
\caption{Average precision, recall and f-measure scores on test folds. Underlined components correspond to the state-of-the-art choices.}
\label{table:results}
\centering
...
\textit{Baseline.} The simplest baseline against which we compare our results
consists in grouping all signatures sharing the same (normalized) surname(s) and
the same (normalized) first given name initial. As it does not require the usage
of any machine learning component, this baseline provides a simple and fast
solution yielding decent results, as reported at the top of
Table~\ref{table:results}

\textit{State-of-the-art.} Most methods proposed in related works have released
...
Sections \ref{choices:blocking}, \ref{choices:linkage}, and
\ref{choices:clustering} now study each component individually and discuss
...
Let us also note that Table~\ref{table:blocking} corroborates the
...
Finally, as discussed previously, the seemingly insignificant step of
...
The best combined settings reduces the error rate by $76.65\%$ in comparison with the baseline. This means that more than three quarters of ambiguous author names are predicted to be resolved using our approach in compare with using \textit{SFI} method a lone after applying \textit{Name Normalization}. In comparison with the state-of-the-art, the error rate reduction is $18.82\%$ which means about fifth of ambiguous author names (\ie, signatures) that is unsolved by the state-of-the-art would be addressed. When we deal with 10 millions of signatures(\ie, the case of the digital library we took the dataset from), this performance enhancements is equivalent to $453000$ more signatures to be resolved in comparison of the baseline which is expensive to resolve with manual curation. In comparison of the state-of-the-art, this is equivalent to $32000$ more signatures to be resolved.

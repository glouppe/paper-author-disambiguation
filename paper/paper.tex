...
subsets $c_i$  each corresponds to the set of all signatures from the same
...
problem.  Our algorithm is made of three parts, as sketched in Figure~\ref{fig:workflow}: (i) a blocking
...
typically reduces from $O(|{\cal S}|^2)$ to $O(\sum_b |{\cal S}_b|^2)$, which becomes much more
...
given name initial (\eg, SFI("Doe, John") $==$ "Doe, J"). Despite satisfactory performance, such
...
\begin{enumerate}
  ...
  last one are contained within given names (\eg, "Martinez Torres, A." and "Torres, A.
  Martinez").
  ...
  \item An author as multiple given names and they are not always all mentioned (\eg,
  "Smith, Jack" and "Smith, A. J.")

  \item An author changed his surname (\eg, because of marrying somebody).
\end{enumerate}
...
stripping accents (\eg, "Jabłoński, Ł" $\rightarrow$ "Jablonski, L") and name
...
group most name variants of the same person in the same block while not
...
later use during clustering to group all signatures from the same author.
...
clusters are known. From these, we evaluate the following semi-supervised cut-off strategies, as illustrated in Figure~\ref{fig:cuts}:
\begin{itemize}

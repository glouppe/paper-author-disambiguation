...
the relevance search of results (e.g., when querying for all publications written by a given author);
...
their publications themselves (e.g., Google Scholar or Inspire-HEP).
Recent efforts to create persistent digital identifiers assigned to researchers (e.g., ORCID or ResearcherID),
with the objective to embed these identifiers in the submission workflow of publishers
or repositories (e.g., Elsevier, arXiv or Inspire-HEP), would univocally solve any disambiguation issue.
...
given name initial (e.g., \emph{SFI}("Doe, John") $==$ "Doe, J").
...
\begin{enumerate}
  ...
  \item An author has multiple surnames and some signatures place the first part of the surname within the given names (e.g., "Martinez Torres, A." and "Torres, A. Martinez").
...
Normalization involves stripping accents (e.g., "Jabłoński, Ł" $\rightarrow$ "Jablonski, L") and name
...
difficult cases when building the training set (e.g., pairs of signatures with
...
\begin{figure*}
\centering
\includegraphics[width=\textwidth]{fig-cuts}
...
As shown in Table~\ref{table:blocking}, the maximum recall (e.g., if within a block, all signatures were clustered optimally) for SFI is $0.9828$.
...
fact it requires almost no computation (e.g., both learning a linkage function
...
\textit{Combined best settings.} When all best settings are combined (e.g.,
...
performance reaches $0.9862$, e.g., the best of all reported results. In particular,
...
is known to be less efficient (e.g., Chinese authors with many homonyms)  indeed

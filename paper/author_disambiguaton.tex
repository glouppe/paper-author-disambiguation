...
function, thereby tailoring disambiguation to non-Western author names whenever necessary. (iii) Showing the importance of balancing negative and positive examples when learning the linkage function.
In addition to releasing the software implementation of our improved pipeline, we also published the verified dataset to ease the comparison with other researchers who are interested in further contribution. 
\end{abstract}
...
\textit{Combined best settings.} When all best settings are combined (\ie, Blocking = NYSIIS, Name normalization, Classifier = Random Forests, Training
